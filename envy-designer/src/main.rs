use std::{path::{Path, PathBuf}, sync::{atomic::AtomicU64, Arc}};

use envy::{LayoutRoot, LayoutTemplate, NodeImplTemplate};
use envy_wgpu::WgpuBackend;
use parking_lot::Mutex;

use crate::widgets::{animator::AnimatorWidget, layout_renderer::{LayoutReference, LayoutRenderer, LayoutRendererCommand}, resource_viewer::{ResourceKind, ResourceViewer, ResourceViewerCommand}};
...
impl egui_dock::TabViewer for AppTabViewer<'_> {
    ...
    fn ui(&mut self, ui: &mut egui::Ui, tab: &mut Self::Tab) {
        match &mut tab.inner {
            ...
            AppTab::Resource(viewer) => self.resource_commands.extend(viewer.show(ui, self.render_state)),
        }
    }
    ...
    fn add_popup(&mut self, ui: &mut egui::Ui, surface: egui_dock::SurfaceIndex, node: egui_dock::NodeIndex) {
        ...
        let templates = root.templates().into_iter().map(|(template, _)| template.to_string()).collect::<Vec<_>>();
        drop(root);
        ...
        ui.menu_button("Open", |ui| {
            ...
            if ui.button("Root Layout").clicked() {
                self.new_tab = Some(NewTab {
                    tab: AppTab::Layout(LayoutRenderer::new(self.root.clone(), self.backend.clone(), self.render_state, LayoutReference::Root)),
                    surface,
                    node
                });
                ui.close();
            ...
            if ui.button("Animator").clicked() {
                self.new_tab = Some(NewTab {
                    tab: AppTab::Animator(AnimatorWidget::new(self.root.clone(), self.backend.clone(), self.render_state)),
                    surface,
                    node
                });
                ui.close();
            ...
            for template in templates.into_iter().filter(|template| !template.is_empty()) {
                if ui.button(&template).clicked() {
                    self.new_tab = Some(NewTab {
                        tab: AppTab::Layout(LayoutRenderer::new(self.root.clone(), self.backend.clone(), self.render_state, LayoutReference::Sublayout(template.as_str()))),
                        surface,
                        node,
                    });
                    ui.close();
        ...
        ui.menu_button("Resources", |ui| {
            ...
            if ui.button("Images").clicked() {
                self.new_tab = Some(NewTab {
                    tab: AppTab::Resource(ResourceViewer::new(self.backend.clone(), ResourceKind::Images)),
                    surface,
                    node
                });
                ui.close();
            ...
            if ui.button("Fonts").clicked() {
                self.new_tab = Some(NewTab {
                    tab: AppTab::Resource(ResourceViewer::new(self.backend.clone(), ResourceKind::Fonts)),
                    surface,
                    node
                });
                ui.close();
        ...
        if !ui.should_close() && ui.button("New Sublayout").clicked() {
            {
                self.root.lock().add_template("new_template", LayoutTemplate::default());
            }
            self.new_tab = Some(NewTab {
                tab: AppTab::Layout(LayoutRenderer::new(self.root.clone(), self.backend.clone(), self.render_state, LayoutReference::Sublayout("new_template"))),
                surface,
                node
            });
            ui.close();
...
impl Application {
    pub fn open(wgpu_render_state: &egui_wgpu::RenderState, path: impl AsRef<Path>) -> Self {
        ...
        let mut backend = WgpuBackend::new(wgpu_render_state.device.clone(), wgpu_render_state.queue.clone(), wgpu_render_state.target_format, widgets::layout_renderer::SAMPLE_COUNT as usize);

        let file_data = std::fs::read(path).unwrap();
        let mut root = envy::asset::deserialize(&mut backend, &file_data);
        root.setup(&mut backend);

        let templates = root.templates().into_iter().map(|(name, _)| name).filter(|name| !name.is_empty()).map(|s| s.to_string()).collect::<Vec<_>>();

        let backend = Arc::new(Mutex::new(backend));
        let root = Arc::new(Mutex::new(root));

        let mut tabs = vec![AppTabWrapper::new(AppTab::Layout(LayoutRenderer::new(root.clone(), backend.clone(), wgpu_render_state, LayoutReference::Root)))];

        for template in templates {
            tabs.push(AppTabWrapper::new(AppTab::Layout(LayoutRenderer::new(root.clone(), backend.clone(), wgpu_render_state, LayoutReference::Sublayout(template.as_str())))));
        }
    ...
    pub fn new(wgpu_render_state: &egui_wgpu::RenderState) -> Self {
        ...
        let tabs = vec![AppTabWrapper::new(AppTab::Layout(LayoutRenderer::new(root.clone(), backend.clone(), wgpu_render_state, LayoutReference::Root)))];
...
impl eframe::App for Application {
    fn update(&mut self, ctx: &egui::Context, frame: &mut eframe::Frame) {
        ctx.request_repaint();
        egui::TopBottomPanel::top("file-bar")
            .show(ctx, |ui| {
        ...
        egui::CentralPanel::default()
            .show(ctx, |ui| {
                ...
                for command in viewer.layout_commands {
                    match command {
                        LayoutRendererCommand::RenameSublayout { old_name, new_name } => {
                            {
                                self.root.lock().rename_template(old_name.as_str(), new_name.as_str());
                            }
                ...
                for command in viewer.resource_commands {
                    match command {
                        ResourceViewerCommand::Add { kind } => {
                            match kind {
                                ResourceKind::Fonts => {
                                    if let Some(file) = rfd::FileDialog::new().set_title("Add Font Resource").add_filter("Font File", &["ttf", "otf"]).pick_file() {
                                ...
                                ResourceKind::Images => {
                                    if let Some(file) = rfd::FileDialog::new().set_title("Add Texture").add_filter("PNG File", &["png"]).pick_file() {
                        ...
                        ResourceViewerCommand::Rename { kind, old_name, new_name } => {
                            let mut root = self.root.lock();
                            let mut backend = self.backend.lock();
                            let templates = root.templates().into_iter().filter(|(name, _)| !name.is_empty()).map(|(name, _)| name.to_string()).collect::<Vec<_>>();
                            match kind {
                                ResourceKind::Fonts => {
                        ...
                        ResourceViewerCommand::Replace { kind, name } => {
                            match kind {
                                ResourceKind::Fonts => {
                                    if let Some(file) = rfd::FileDialog::new().set_title("Add Font Resource").add_filter("Font File", &["ttf", "otf"]).pick_file() {
                                        let file_data = std::fs::read(&file).unwrap();
                ...
                if let Some(tab) = viewer.new_tab {
                    self.dock_state.get_surface_mut(tab.surface).unwrap()[tab.node].append_tab(AppTabWrapper::new(tab.tab));
                }
            });
    }
}
...
fn main() {
    ...
    eframe::run_native("Envy Designer", eframe::NativeOptions::default(), Box::new(|ctx| {
        Ok(Box::new(Application::new(ctx.wgpu_render_state.as_ref().unwrap())))
    })).unwrap();
}
