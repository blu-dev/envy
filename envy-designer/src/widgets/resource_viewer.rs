            ...
            match self.kind {
                ResourceKind::Images => {
                    for image_name in backend.iter_texture_names() {
                        let (rect, response) = ui.allocate_exact_size(
                            egui::Vec2::new(ui.available_width(), Self::TILE_HEIGHT),
                            egui::Sense::click(),
                        );
                        ...
                        let img_space = egui::Rect::from_min_size(
                            rect.min + egui::Vec2::splat(5.0),
                        ...
                        if let Some(rename) =
                            self.rename.as_mut().filter(|ctx| ctx.old == image_name)
                        {
                            let width = (ui.available_width() - 20.0).max(0.0);
                            let is_done = ui.scope_builder(
                                egui::UiBuilder::new().max_rect(egui::Rect::from_min_size(
                                    rect.min + egui::Vec2::new(80., 15.),
                                    egui::Vec2::new(width, 16.0),
                                )),
                                |ui| ui.text_edit_singleline(&mut rename.new).lost_focus(),
                            );
                        ...
                        response.context_menu(|ui| {
                            if ui.button("Remove").clicked() {
                                ...
                                let _ = self.thumbnails.remove(image_name);
                                commands.push(ResourceViewerCommand::Replace {
                                    kind: self.kind,
                                    name: image_name.to_string(),
                                });
                                ui.close();
                ...
                ResourceKind::Fonts => {
                    for font_name in backend.iter_font_names() {
                        let (_id, rect) = ui.allocate_space(egui::Vec2::new(
                            ui.available_width(),
                            Self::TILE_HEIGHT,
                        ));
                        ...
                        if let Some(rename) =
                            self.rename.as_mut().filter(|ctx| ctx.old == font_name)
                        {
                            let width = (ui.available_width() - 20.0).max(0.0);
                            let is_done = ui.scope_builder(
                                egui::UiBuilder::new().max_rect(egui::Rect::from_min_size(
